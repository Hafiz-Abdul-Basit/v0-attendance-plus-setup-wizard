      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_280px] gap-6">
        ...
        <div className="min-w-0 space-y-4">
          ...
          <div>
            ...
            <div className="text-sm text-gray-800 bg-white border border-gray-200 rounded-md p-3 max-h-72 overflow-auto whitespace-pre-wrap font-mono leading-relaxed">
              {task.description || (
          ...
          <div>
            ...
            {attachments.length > 0 ? (
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
                {attachments.map((a) => {
                  // Two URLs per attachment: one inline-preview (default
                  // Content-Disposition), one explicit download. A single
                  // link with `download=` is unreliable for cross-origin
                  // streaming responses, so we render both actions.
                  ...
                  return (
                    ...
                    >
                      <span className="text-base shrink-0 self-center" aria-hidden>
                        {fileIcon(a.name)}
                      </span>
                      <a
                        href={previewUrl}
                        ...
                        title={a.comment ? `Open ${a.name} — ${a.comment}` : `Open ${a.name}`}
              ...
              // List endpoint confirmed: zero attachments on this work item.
              <div className="text-xs text-gray-500 italic flex items-center gap-1.5">
                <FileText className="w-3.5 h-3.5" />
                No attachments on this work item.
              </div>
            ) : task.relationsUnavailable ? (
              // Fallback path was taken upstream. Every `$expand=relations`
              // (and the legacy `expand=relations` variant) was rejected.
              // The single most common cause is a missing `vso.work_full`
              // PAT scope — that scope is required to read work-item
              // relations via the REST API. We name it explicitly so the
              // admin can fix the PAT configuration without having to
              // read server logs.
              <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900 space-y-1.5">
                ...
                <p>
                  ...
                  (including attachments). The rest of this work item
                  loaded fine — only attachments are affected.
                </p>
                <p className="text-amber-800">
                  ...
                  with <strong>Full Access</strong> selected, then refresh
                  this page.
                </p>
                <p className="text-amber-800">
                  To confirm the diagnosis without reading server logs,
                  open{" "}
                  <a
              ...
              // Generic fallback — shouldn't normally render now that we
              // distinguish `relationsUnavailable`, but kept for safety.
              <div className="text-xs text-gray-500 italic flex items-center gap-1.5">
                <FileText className="w-3.5 h-3.5" />
                Attachments are unavailable for this work item. Re-open
                the row to retry.
              </div>
            )}
          </div>
